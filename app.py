...
if run_lstm:
    ...
    def run_lstm_model(ticker, lookback):
        try:
            ...
            # Predict future N days using rolling forecast
            def predict_future(n_days):
                future_prices = []
                current_seq = scaled[-lookback:].flatten().tolist()
                for _ in range(n_days):
                    input_seq = np.array(current_seq[-lookback:]).reshape(1, lookback, 1)
            ...
            future_7  = predict_future(7)
            future_30 = predict_future(30)
            next_price = future_7[0]
    ...
    if result is None:
        ...
        def make_forecast_df(future_arr, n_days):
            ...
            while count < n_days:
                ...
                if d.weekday() < 5:   # skip weekends
                    ...
                    count += 1
            df_f = pd.DataFrame({
                'Date': [fd.strftime('%d %b %Y (%a)') for fd in future_dates],
                'Predicted Price ($)': [f"${p:.2f}" for p in future_arr],
                'Change from Today': [
                    f"{'▲' if p > curr_p else '▼'} ${abs(p - curr_p):.2f} ({abs((p-curr_p)/curr_p)*100:.1f}%)"
                    for p in future_arr
                ]
            })
            return df_f, future_dates, future_arr

        with tab1:
            df_7, dates_7, prices_7 = make_forecast_df(result['future_7'], 7)
            st.dataframe(df_7, use_container_width=True, hide_index=True)

        with tab2:
            df_30, dates_30, prices_30 = make_forecast_df(result['future_30'], 30)
            st.dataframe(df_30, use_container_width=True, hide_index=True)

        # ── Full prediction chart with future ────────────────────────────────
        ...
        _, future_dates_plot, future_prices_plot = make_forecast_df(result['future_30'], 30)
        ax.plot(future_dates_plot, future_prices_plot,
                color=ORANGE, linewidth=2, linestyle=':', label='30-Day Forecast')
        ax.fill_between(future_dates_plot,
                        [p * 0.97 for p in future_prices_plot],
                        [p * 1.03 for p in future_prices_plot],
                        alpha=0.12, color=ORANGE, label='±3% Confidence Band')
        ax.scatter(future_dates_plot[0], future_prices_plot[0],
                   color=ORANGE, s=100, zorder=6)

        ax.axvline(x=last_date, color='#5a5a8a', linestyle='--',
                   linewidth=1, alpha=0.6, label='Today')

        ax.set_title(f'{primary_ticker} — LSTM Prediction + 30-Day Forecast', fontsize=11, fontweight='bold')
